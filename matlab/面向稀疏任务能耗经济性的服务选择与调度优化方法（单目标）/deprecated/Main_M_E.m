...
Populations_Fitness = cell(gen_max, 1); % 记录种群适应值的元胞矩阵，用于记录每一代个体的适应值
...
while (gen <= gen_max)
    ...
    Energy_dimensionless = dimensionless_Energy(Energy, Energy_max); % 预热能量无量纲化
    ...
    [Quality_dimensionless, Cost_dimensionless, Time_dimensionless] = dimensionless_bottom2(Quality, Cost, Time, Time_required_max, Cost_required_max, Quality_required_min, Time_min, Cost_min, Quality_max); % ���������ٻ�

    %% 计算适应度
    Fitness = fitness_single_object(Energy_dimensionless, Quality_dimensionless, Cost_dimensionless, Time_dimensionless, w_Energy, w_Quality, w_Cost, w_Time);
